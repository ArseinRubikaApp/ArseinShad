...
class Messenger:
    def __init__(self, Sh_account: str = None, keyAccount: str = None, TypePlat = None,session_file: str = None):
        
        if session_file:
            with open(session_file.rsplit('.', 1)[0] + ".json", "r", encoding = "utf-8") as f:
                data = load(f)
...
def signIn(platforms, numberphone: str, codehash, phone_code, save=None):
    ...
    if platforms and numberphone and codehash and phone_code:
        ...
        if GetDataSignIn.get("data").get("status") == "OK":
            data_account = dict(
                Auth = encoderjson.changeAuthType(encoderjson.decryptRsaOaep(
